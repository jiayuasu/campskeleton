...
//        direction.normalize();
     ...
     *
     * @throws Error if the three planes do not intersect at a single point (two or more of them
     *     are parallel or coincident). Note this is not a RuntimeException: the original Jama-based
     *     implementation threw RuntimeException("Matrix is singular.") here, which let callers such
     *     as org.twak.camp.Edge#collide fall back to a parallel-edge construction. With the Error,
     *     the degenerate event is abandoned in Skeleton#skeleton instead. Restoring the
     *     RuntimeException was tried and produced worse skeletons (edges far outside the input,
     *     crossing arcs) on rasterised outlines, so the abandon-the-event behaviour is kept.
     */
